...
export const CreateRewardModal: React.FC<CreateRewardModalProps> = ({ isOpen, onClose }) => {
  ...
  const isRewardValid = selectedReward !== 'cash' || (rewardValue.trim() !== '' && !isNaN(Number(rewardValue)) && Number(rewardValue) > 0);
                                  ...
                                  <div className="absolute top-[calc(100%+4px)] left-0 w-full bg-white border border-[#E3E3E3] rounded-lg shadow-lg z-[1000] p-1 flex flex-col gap-0.5">
                                    {durationOptions.map((dur) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          <span>{dur}</span>
                                          {isDurSelected && <Check size={16} strokeWidth={2} className="text-[#C530C5]" />}
                                        </button>
                                      );
                                    })}
                                  </div>
                    ...
                    return (
                      <div key={option.id} className="flex flex-col">
                        <button
                          ...
                          className={`flex items-center justify-between w-full px-3 py-2 text-[14px] font-inter text-left rounded-lg transition-colors duration-150 ${isDisabled
